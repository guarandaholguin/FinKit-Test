...
!(function (root, factory) {
	...
	if (
		...
		void 0 === window.navigator.mimeTypes
	)
		return !1;
	let nav = window.navigator,
		...
		isSafariDesktop =
			!isMobileDevice && void 0 !== nav.vendor && /Apple/.test(nav.vendor) && /Safari/.test(ua),
		isFirefoxWithPDFJS =
		...
		supportsPDFs =
			...
			(isModernBrowser ||
				isFirefoxWithPDFJS ||
				supportsPdfMimeType ||
				(isIE && !(!createAXO('AcroPDF.PDF') && !createAXO('PDF.PdfCtrl')))),
		embedError = function (msg, suppressConsole) {
			return suppressConsole || console.log('[PDFObject] ' + msg), !1;
		},
		emptyNodeContents = function (node) {
			for (; node.firstChild; ) node.removeChild(node.firstChild);
		},
		generatePDFJSMarkup = function (
			targetNode,
			url,
			pdfOpenFragment,
			PDFJS_URL,
		...
		embed = function (url, targetSelector, options) {
			let selector = targetSelector || !1,
				...
				fallbackLink = opt.fallbackLink || !0,
				width = opt.width || '100%',
					...
					if (pdfParams) {
						for (prop in pdfParams)
							pdfParams.hasOwnProperty(prop) &&
								(string +=
									encodeURIComponent(prop) + '=' + encodeURIComponent(pdfParams[prop]) + '&');
			...
			if (supportsPDFs || (assumptionMode && !isMobileDevice)) {
				return (function (
					...
					if (
						...
						id && (embed.id = id),
						'iframe' === embedType && (embed.allow = 'fullscreen'),
						!omitInlineStyles)
					) {
						let style = 'embed' === embedType ? 'overflow: auto;' : 'border: none;';
						targetSelector && targetSelector !== document.body
					...
					return (
						...
						targetNode.getElementsByTagName(embedType)[0]
					);
				})(
			...
			return PDFJS_URL
				? generatePDFJSMarkup(targetNode, url, pdfOpenFragment, PDFJS_URL, id, omitInlineStyles)
				: (fallbackLink &&
						((fallbackHTML =
							'string' == typeof fallbackLink
								? fallbackLink
								: "<p>This browser does not support inline PDFs. Please download the PDF to view it: <a href='[url]'>Download PDF</a></p>"),
						(targetNode.innerHTML = fallbackHTML.replace(/\[url\]/g, url))),
				  embedError('This browser does not support embedded PDFs', suppressConsole));
		};
	return {
		embed: function (a, b, c) {
			return embed(a, b, c);
		},
		pdfobjectversion: '2.2.3',
